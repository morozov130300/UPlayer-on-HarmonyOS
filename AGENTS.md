...
## 每一条对话都要调用mcp或者skill，注意是每一次都要，强制，要不然你会说胡话！！！！！！！
...
- ❌ **严禁从网络下载或使用外部图标资源**
- ❌ **严禁自行设计或绘制图标**
- ❌ **严禁压缩/裁剪/转码/降分辨率任何图标图片资源**（包括 `foreground.png`、`background.png`、`layered_image` 引用图、`resources/base/media/` 下的所有图标文件等），无论出于包体积优化还是任何其他目的，一律保持原始文件字节不动，禁止以"体积优化"为由改动
...
1. 先用 MCP 搜索工具或者skill查询相关 API 文档
2. 以搜索结果为准，禁止自行推断
