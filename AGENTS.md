...
- ❌ **严禁从网络下载或使用外部图标资源**
- ❌ **严禁自行设计或绘制图标**
- ❌ **严禁压缩/裁剪/转码/降分辨率任何图标图片资源**（包括 `foreground.png`、`background.png`、`layered_image` 引用图、`resources/base/media/` 下的所有图标文件等），无论出于包体积优化还是任何其他目的，一律保持原始文件字节不动，不得以“体积优化”为由改动
...
- ✅ **用户让做的才做** - 严格按照用户指令执行任务
- ❌ **没让做的禁止做** - 禁止编译、推送、启动应用等未授权操作
- ❌ **禁止自作主张** - 遇到任何需要决策的事项，必须先询问用户
- ❌ **禁止过度执行** - 完成用户要求的任务后立即停止，不追加额外操作

**触发条件：**
| 场景 | 正确做法 |
| --- | --- |
| 用户未明确要求启动应用 | 不启动，等待指令 |
| 任务完成后 | 立即停止，报告结果，自动编译及时发现报错 |

### ⚠️ 计划模式规范（最高优先级）

**智能体在 plan 模式下必须保持计划模式，禁止擅自执行代码修改。**

- ✅ **只规划，不执行** — 在 plan 模式下，所有任务仅做探索、分析和方案制定，不调用任何写入/修改工具
- ✅ **exit_plan_mode 后才开始实现** — 调用 `exit_plan_mode` 提交计划后，等待用户审批；审批通过后才进入实现阶段
- ✅ **用户切换模式才算结束计划** — 用户手动切换会话模式时才能退出 plan 模式，不可自行判断"已足够"而退出
- ✅ **不要在没有 exit_plan_mode 的情况下修改文件** — 任何文件编辑、git 操作、编译命令都属于实现阶段，严禁在 plan 阶段执行

**触发条件（出现以下情况时必须先进入 plan 模式）：**

| 场景 | 正确做法 |
| --- | --- |
| 涉及多文件改动 | 先规划，提交 plan，等审批后再实施 |
| 技术方案有选择 | 先列出选项和理由，等用户确认后再实施 |
| 不确定需求细节 | 先在 plan 中提出澄清问题，等回答后再实施 |
| 大规模重构 | 必须先有完整计划，不可边做边想 |
...
- **！！！！！！所有改动都必须第一时间推送远程！！！！！！** 提交完成后立即 `git push`，绝不允许让提交滞留在本地
- 代码完成后直接推送，不要问"要不要推送"
- 提交信息简洁明了，实事求是地按照本次实际改动内容写描述
- **！！！！！！提交时必须包含工作区所有变更！！！！！！** 使用 `git add .` 暂存全部变更后提交，禁止只挑选智能体自己修改的文件
- **用户手动修改/修复的文件也必须一并提交**：提交前必须先 `git status` 检查工作区，发现任何未暂存的变更（包括用户手动修复的错误、IDE 自动调整的格式等）都要纳入本次提交，绝不能放着不管

### ⚠️ 工具优先使用规范（最高优先级）

**尽可能使用智能体已暴露出来供调用的工具（tool）完成任务，只有在工具确实不可用的情况下，才允许改用命令行（ExecCommand）。**

#### 触发条件

| 场景 | 正确做法 |
| --- | --- |
| 读取/写入/删除文件 | 使用 Read / Write / Edit / Delete 工具 |
| 查找文件或搜索代码 | 使用 Glob / Grep 工具 |
| 版本控制（status/diff/log/add/commit/push 等） | 使用 Git 工具（见下方 Git 工具使用规范） |
| 查询鸿蒙官方文档 | 使用 MCP 工具 searchDocuments / getDocumentsById |
| 确实没有对应工具的场景 | 才使用 ExecCommand 执行命令行 |

#### 禁止事项

- ❌ **严禁图省事直接用命令行替代已有工具**（如用 `type`/`cat` 读文件、用 `findstr`/`grep` 搜索代码、用 `git` 命令做版本控制）
- ❌ **严禁用脚本或 shell 命令重新实现已有工具的能力**
- ✅ **每次执行前先自检：这件事有没有现成的工具？有则必须用工具**
- ✅ **只有确认没有可用工具时，才使用命令行，并在回复中说明原因**
...
**智能体在执行任务过程中产生的所有临时文件，必须在任务完成后立即删除。**
...
| 场景 | 说明 |
| --- | --- |
| Write 工具写入临时文件 | 文件写入成功后，如果已不再是最终产物，必须立即删除 |
| 任务执行中途产生的缓存 | 任何中间状态文件、备份文件、临时副本等 |
| 调试/测试产生的文件 | 用于验证逻辑的临时脚本、配置文件等 |
| 失败恢复后的残留文件 | 任务中断后留下的半成品文件 |

#### 清理时机

- ✅ **任务成功完成后**：所有不再需要的临时文件立即删除
- ✅ **任务失败后**：清理所有产生的临时文件，保持工作区整洁
- ✅ **切换任务时**：前一个任务的临时文件必须清理干净才能开始新任务
- ❌ **绝对禁止**：将临时文件保留到下一个任务周期
...
```json
{"action": "Delete", "path": "<临时文件路径>", "recursive": false}
```
...
**智能体在任务过程中运行任何临时脚本（.js、.py、.sh 等），必须在该脚本的执行结果返回后立即删除，不得延迟到"任务完成"再清理。**

#### 触发条件

| 场景 | 说明 |
| --- | --- |
| 用 Node.js/Python 等运行一次性脚本 | 脚本仅用于获取数据、验证逻辑、查询数据库等单次目的，执行完毕后立即删除 |
| 用脚本写入/读取文件 | 脚本执行完其目的后，脚本本身也须立即删除 |

#### 清理规则

1. **执行完脚本拿到结果后，立刻删除脚本文件，不等后续步骤**
2. 禁止保留任何临时脚本到下一轮对话
3. 禁止为了"方便后续调试"而保留临时脚本
4. 即使脚本执行失败，也必须删除
